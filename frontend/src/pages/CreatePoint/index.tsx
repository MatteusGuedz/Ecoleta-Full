...
function handleSelectItem(id: number){
    const alreadySelected = itemSelect.findIndex(item => item === id);

    if( alreadySelected >= 0){
      const filteredItems = itemSelect.filter(item => item !== id);
      setItemSelect(filteredItems)
    } else {
...
;
  ...
  const data  = new  FormData();
    ...
    alert('Ponto de Coleta Criado para ' + city );
       ...
       <fieldset>
         ...
         <ul className="items-grid">
           {items.map(item => (
            ...
            className={itemSelect.includes(item.id) ? 'selected' : ''}
            >
